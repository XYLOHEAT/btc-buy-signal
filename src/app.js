/* UI. Styles are StyleX (styles.js, tokens.stylex.js); build.mjs bundles this into /app.js and inlines the CSS
   into index.html (ADR-021). indicators.js stays a plain script (window.Indicators), Chart.js comes from the CDN. */
import * as stylex from "@stylexjs/stylex";
import { sx, lightTheme, darkTheme } from "./styles.js";
const x=(...roles)=>stylex.props(...roles).className; // one element's class list from its style roles
...
const T={
 th:{
  ...
  priceLive:(n,src)=>`ราคาสด ${n} · ${src}`, priceOff:"ดึงราคาสดไม่ได้ แสดงราคาปิดล่าสุดแทน", d24:"24 ชม.",
 ...
 en:{
  ...
  priceLive:(n,src)=>`Live price ${n} · ${src}`, priceOff:"Live price unavailable, showing the last close", d24:"24h",
...
/* ---------- theme ---------- */
const SUN=`<svg class="${x(sx.icon)}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M2 12h2M20 12h2M5 5l1.4 1.4M17.2 17.2L18.6 18.6M18.6 5.4L17.2 6.8M6.8 17.2L5.4 18.6"/></svg>`;
const MOON=`<svg class="${x(sx.icon)}" viewBox="0 0 24 24" fill="currentColor"><path d="M21 12.8A9 9 0 1 1 11.2 3a7 7 0 0 0 9.8 9.8z"/></svg>`;
let THEME="auto";
const DARK=()=>THEME==="dark"||(THEME==="auto"&&matchMedia("(prefers-color-scheme: dark)").matches);
function applyTheme(mode){THEME=mode;document.documentElement.className=mode==="dark"?x(darkTheme):mode==="light"?x(lightTheme):"";
...
async function load(full){
  ...
  if(!COMP)app.dataset.phase="loading";
  announce(L().loading);
  const tp=ticker();
  try{
...
function showErr(t){
  const l=L();lastT=t;
  if(!COMP){document.getElementById("app").dataset.phase="failed";document.getElementById("asof").innerHTML=t?renderPrice(t,t.price):"";}
...
function spark(key,bands){
  ...
  return`<svg class="${x(sx.spark)}" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" aria-hidden="true">${bl}<path d="${d}" fill="none" style="stroke:var(--ink)" stroke-width="1.6" stroke-linejoin="round"/></svg>`;
}
...
function head(id,str){const [t,sub]=str.split(" · "),h=document.getElementById(id);h.textContent=t;
  if(sub){const sm=document.createElement("small");sm.className=x(sx.lblSub);sm.textContent=sub;h.appendChild(sm);}}
...
function render(t){
  ...
  document.getElementById("gaugeSvg").setAttribute("aria-label",l.scoreAria(Math.round(SNAP.overall)));
...
function renderRows(){
  ...
  Indicators.INDICES.forEach(s=>{
    ...
    btn.onclick=()=>{const open=btn.getAttribute("aria-expanded")!=="true";btn.setAttribute("aria-expanded",open);info.hidden=!open;};
    rows.appendChild(el);
  });
}
...
function opts(id,items,cur,pick,role=sx.opt){const h=document.getElementById(id);h.innerHTML="";
...
const tbl=(cols,rows)=>cols.map((h,i)=>`<div class="${x(sx.cell,sx.th,i&&sx.thNum)}">${h}</div>`).join("")+
  rows.map(([r,...v])=>`<div class="${x(sx.cell,sx.tr)}">${r}</div>`+v.map((val,i)=>`<div class="${x(sx.cell,sx.num)}" data-l="${cols[i+1]}">${val}</div>`).join("")).join("");
...
<header class="${x(sx.mast)}">
  ...
  <div class="${x(sx.ctrls)}">
    <button class="${x(sx.cbtn)}" id="langBtn" title="ภาษา / Language">EN</button>
    <button class="${x(sx.cbtn)}" id="themeBtn" aria-label="theme"></button>
    <button class="${x(sx.cbtn)}" id="refresh" aria-label="refresh">↻</button>
  </div>
</header>
<main class="${x(sx.cols)}">
<div class="${x(sx.colA)}">
  <section class="${x(sx.verdict)}">
    <div class="${x(sx.gauge)}">
      <svg class="${x(sx.gaugeSvg)}" id="gaugeSvg" width="196" height="196" viewBox="0 0 196 196" role="img" aria-label="accumulation score">
        <circle class="${x(sx.track)}" cx="98" cy="98" r="88"/><circle class="${x(sx.arc)}" id="arc" cx="98" cy="98" r="88"/>
      </svg>
      <div class="${x(sx.ctr)}" aria-hidden="true"><div class="${x(sx.score)}" id="score">–<span class="${x(sx.scoreMax)}">/100</span></div></div>
    </div>
    <div class="${x(sx.vtext)}">
      <div class="${x(sx.zone)}" id="zone">–</div>
      <div class="${x(sx.zoneTh,sx.sk,sx.skText,sx.skBar,sx.hZoneth)}" id="zoneth" data-sk></div>
      <div class="${x(sx.tape)}"><span id="price">$–</span><span class="${x(sx.chg)}" id="chg"></span></div>
      <div class="${x(sx.asof,sx.sk,sx.skText,sx.skBar,sx.hAsof)}" id="asof" data-sk></div>
      <div class="${x(sx.alert)}" id="err" role="alert" hidden><p class="${x(sx.alertP)}" id="errmsg"></p><button class="${x(sx.cbtn,sx.retry)}" id="retry"></button></div>
    </div>
  </section>
  <!-- the recommendation, said once; then the few facts it rests on -->
  <dl class="${x(sx.plan)}">
    <div><dt class="${x(sx.planDt)}" id="ins-action-l"></dt><dd class="${x(sx.planDd,sx.planAct,sx.sk,sx.skText,sx.hDca)}" id="ins-dca" data-sk></dd></div>
...
<footer class="${x(sx.foot)}" id="foot"></footer>`;

/* ---------- init ---------- */
const APP=document.getElementById("app");
APP.className=x(sx.wrap); // data-phase on #app drives the loading skeleton (styles.js)
APP.innerHTML=shell();document.getElementById("live").className=x(sx.sr);
